...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) {
  ...
  // If the list is empty or the new block's size is less than or equal to the head block size
  ...
  node_t *current = l->head;
  while (current->next != NULL && 
         !compareSize((newblk->end - newblk->start + 1), current->next->blk)) {
    current = current->next;
  }
